...
import {
  getPersonnel,
  getSchools,
  recordPersonnelAttendance,
  updatePersonnelAttendance,
  deletePersonnelAttendance,
  listPersonnelAttendanceByPersonnelRange,
  getErrorMessage,
} from '../api/client';
...
const PersonnelAttendance = () => {
  ...
  const [loading, setLoading] = useState({ personnel: false, attendance: false });
  const [error, setError] = useState('');
  const [modal, setModal] = useState({ open: false, personnelId: null, dateStr: null });
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      setLoading((p) => ({ ...p, personnel: true, attendance: true }));
      setError('');
      try {
        const res = await getPersonnel(1, 200, { school_id: schoolId });
        const list = Array.isArray(res.data) ? res.data : (res.data?.personnel ?? res.data?.data ?? []);
        if (!active) return;
        setPersonnelList(list);
        if (!list.length) {
          setAttendanceMap({});
          setLoading((p) => ({ ...p, personnel: false, attendance: false }));
          return;
        }
        ...
        const map = {};
        const ranges = list.map((p) => listPersonnelAttendanceByPersonnelRange(p.id, from, to));
        const results = await Promise.all(ranges);
        results.forEach((res, idx) => {
          const pId = list[idx].id;
          const records = res.data?.data || res.data || [];
          map[pId] = {};
          records.forEach((r) => {
            const d = r.attendance_date ? new Date(r.attendance_date).toISOString().slice(0, 10) : '';
            if (d) map[pId][d] = { id: r.id, status: r.status };
          });
        });
        if (active) {
  ...
  const recordedBy = user?.first_name && user?.last_name ? `${user.first_name} ${user.last_name}` : user?.email || 'Admin';

  const openModal = (personnelId, dateStr) => {
    setModal({ open: true, personnelId, dateStr });
  };

  const closeModal = () => {
    if (!submitting) setModal({ open: false, personnelId: null, dateStr: null });
  };

  const handleSave = async (status) => {
    if (!schoolId || !modal.personnelId || !modal.dateStr) return;
    const { personnelId, dateStr } = modal;
    setSubmitting(true);
    setError('');
    const current = attendanceMap[personnelId]?.[dateStr];
    try {
      if (status === '') {
        if (current?.id) {
          await deletePersonnelAttendance(current.id);
        }
        setAttendanceMap((prev) => {
          const next = { ...prev };
          if (next[personnelId]) {
            const pNext = { ...next[personnelId] };
            delete pNext[dateStr];
            next[personnelId] = pNext;
          }
          return next;
        });
      } else if (current?.id) {
        const res = await updatePersonnelAttendance(current.id, { status, remarks: '' });
        const updatedId = res.data?.data?.id || current.id;
        setAttendanceMap((prev) => ({
          ...prev,
          [personnelId]: {
            ...prev[personnelId],
            [dateStr]: { id: updatedId, status },
          },
        }));
      } else {
          const res = await recordPersonnelAttendance({
  ...
  const getStatus = (pId, dateStr) => {
    return attendanceMap[pId]?.[dateStr]?.status || '';
  };
  ...
  if (!schoolId) {
    return (
      <div style={{ padding: '32px 16px', maxWidth: 520, margin: '0 auto' }}>
        <h1 style={{ margin: '0 0 16px', fontSize: 22, fontWeight: 800, color: '#0e1f38' }}>Personnel Attendance</h1>
        <p style={{ margin: '0 0 20px', color: '#666', fontSize: 14 }}>
          Select a school to register daily attendance for personnel.
        </p>
        <label style={{ display: 'block', marginBottom: 6, fontSize: 12, fontWeight: 700, color: '#555', textTransform: 'uppercase', letterSpacing: '0.07em' }}>
          School
        </label>
        <select
          value={schoolId}
  ...
  return (
    <div style={{ padding: '24px 16px', maxWidth: 1200, margin: '0 auto' }}>
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ margin: '0 0 6px', fontSize: 22, fontWeight: 800, color: '#0e1f38' }}>Personnel Attendance</h1>
      ...
      >
        <div
          style={{
            display: 'grid',
            gridTemplateColumns: `140px repeat(${workdays.length}, minmax(38px, 1fr))`,
            minWidth: workdays.length * 42 + 140,
          }}
        >
          <div
            style={{
          ...
          {!loading.personnel && personnelList.length === 0 && (
            <div
              style={{
                gridColumn: `1 / -1`,
                padding: '40px',
            ...
            >
              No personnel found for this school.
            </div>
          )}
          ...
          {personnelList.map((p) => (
            <>
              ...
              >
                ...
                <div style={{ fontSize: 11, color: '#888' }}>{p.staff_id}</div>
              </div>
              {workdays.map((d) => {
                const st = getStatus(p.id, d.dateStr);
                const styled = statusStyle(st);
                return (
                  <div
                    key={d.dateStr}
                    onClick={() => openModal(p.id, d.dateStr)}
                    style={{
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
